...
static void aw869x_play_go(struct aw_haptic *aw_haptic, bool flag)
{
	...
	if (flag) {
		haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_GO, AW869X_BIT_GO_MASK,
					 AW869X_BIT_GO_ENABLE);
		aw_haptic->kpre_enter_time = ktime_get();
	} else {
		aw_haptic->kcurrent_time = ktime_get();
		aw_haptic->interval_us = ktime_to_us(ktime_sub(aw_haptic->kcurrent_time,
							       aw_haptic->kpre_enter_time));
		if (aw_haptic->interval_us < 2000) {
			aw_info("aw_haptic->interval_us=%d < 2000", aw_haptic->interval_us);
...
static uint8_t aw869x_get_glb_state(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_GLB_STATE, &state, AW_I2C_BYTE_ONE);
...
static int aw869x_check_ram_data(struct aw_haptic *aw_haptic, uint8_t *cont_data,
				   ...
				   struct aw_haptic_container *awinic_cont)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_FIFO_AEH, fifo_adr, AW_I2C_BYTE_FOUR);
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_FIFO_AEH, fifo_adr, AW_I2C_BYTE_FOUR);
	...
	while (i < awinic_cont->len) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW869X_REG_RAMDATA, &awinic_cont->data[i], len);
		i += len;
	}
#ifdef AW_CHECK_RAM_DATA
	/* ram check */
	...
	while (i < awinic_cont->len) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW869X_REG_RAMDATA, ram_data, len);
		ret = aw869x_check_ram_data(aw_haptic, &awinic_cont->data[i], ram_data, len);
		if (ret < 0)
			break;
		i += len;
	}
 ...
 *
 *****************************************************/
static void aw869x_bst_mode_config(struct aw_haptic *aw_haptic, uint8_t boost_mode)
{
	switch (boost_mode) {
...
static void aw869x_irq_clear(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_SYSINT, &reg_val, AW_I2C_BYTE_ONE);
	aw_dbg("reg SYSINT=0x%x", reg_val);
}

static void aw869x_active(struct aw_haptic *aw_haptic)
{
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_SYSCTRL, AW869X_BIT_SYSCTRL_WORK_MODE_MASK,
				 AW869X_BIT_SYSCTRL_ACTIVE);
	aw869x_irq_clear(aw_haptic);
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_SYSINTM, AW869X_BIT_SYSINTM_UVLO_MASK,
...
static int aw869x_judge_rtp_going(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_SYSCTRL, &reg_val, AW_I2C_BYTE_ONE);
...
static void aw869x_set_wav_seq(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t seq)
{
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_WAVSEQ1 + wav, &seq, AW_I2C_BYTE_ONE);
}
...
static void aw869x_get_ram_data(struct aw_haptic *aw_haptic, char *buf)
{
	...
	while (i < aw_haptic->ram.len) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW869X_REG_RAMDATA, buf + i, size);
		i += size;
	}
}

static void aw869x_get_first_wave_addr(struct aw_haptic *aw_haptic, uint8_t *wave_addr)
{
	uint8_t reg_array[3] = {0};

	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_RAMDATA, reg_array, AW_I2C_BYTE_THREE);
...
static void aw869x_set_ram_addr(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_RAMADDRH, ram_addr, AW_I2C_BYTE_TWO);
}

static void aw869x_set_wav_loop(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t loop)
{
	uint8_t tmp = 0;

	if (wav % 2) {
		tmp = loop << 0;
		haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_WAVLOOP1 + (wav / 2),
...
static size_t aw869x_get_wav_loop(struct aw_haptic *aw_haptic, char *buf)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_WAVLOOP1, reg_val, AW_I2C_BYTE_FOUR);
...
static uint8_t aw869x_get_prctmode(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_PRLVL, &reg_val, AW_I2C_BYTE_ONE);
	reg_val >>= 7;
...
static void aw869x_set_rtp_data(struct aw_haptic *aw_haptic, uint8_t *data, uint32_t len)
{
...
static void aw869x_set_bst_vol(struct aw_haptic *aw_haptic, uint32_t bst_vol)
{
	...
	reg_val = AW869X_BST_VOL_FORMULA(bst_vol);
...
{
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_DATDBG, &gain, AW_I2C_BYTE_ONE);
}
 ...
 *
 *****************************************************/
static void aw869x_protect_config(struct aw_haptic *aw_haptic, uint8_t prtime, uint8_t prlvl)
{
	uint8_t reg_val = 0;

	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_PWMPRC, AW869X_BIT_PWMPRC_PRC_MASK,
				 AW869X_BIT_PWMPRC_PRC_DISABLE);
	if (prlvl != 0) {
		/* Enable protection mode */
		aw_info("enable protection mode");
		reg_val = AW869X_BIT_PRLVL_PR_ENABLE | (prlvl & (~AW869X_BIT_PRLVL_PRLVL_MASK));
		haptic_hv_i2c_writes(aw_haptic, AW869X_REG_PRLVL, &reg_val, AW_I2C_BYTE_ONE);
		haptic_hv_i2c_writes(aw_haptic, AW869X_REG_PRTIME, &prtime, AW_I2C_BYTE_ONE);
	} else {
		/* Disable */
		aw_info("disable protection mode");
		haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_PRLVL, AW869X_BIT_PRLVL_PR_MASK,
 ...
 *
 *****************************************************/
static int aw869x_offset_cali(struct aw_haptic *aw_haptic)
{
	uint8_t reg_val = 0;
	uint32_t cnt = 2000;

	aw869x_ram_init(aw_haptic, true);
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_DETCTRL, AW869X_BIT_DETCTRL_DIAG_GO_MASK,
				 AW869X_BIT_DETCTRL_DIAG_GO_ENABLE);
	while (cnt--) {
		haptic_hv_i2c_reads(aw_haptic, AW869X_REG_DETCTRL, &reg_val, AW_I2C_BYTE_ONE);
		if ((reg_val & 0x01) == 0) {
			aw869x_ram_init(aw_haptic, false);
			return 0;
		}
...
static void aw869x_trig_param_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_TRG1_WAV_P, trig_seq, AW_I2C_BYTE_SIX);
}
...
static uint8_t aw869x_get_trim_lra(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_TRIM_LRA, &reg_val, AW_I2C_BYTE_ONE);
	reg_val &= 0x3F;
...
static void aw869x_get_vbat(struct aw_haptic *aw_haptic)
{
	...
	aw869x_ram_init(aw_haptic, true);
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_DETCTRL, AW869X_BIT_DETCTRL_VBAT_GO_MASK,
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_VBATDET, &reg_val, AW_I2C_BYTE_ONE);
...
static ssize_t aw869x_get_reg(struct aw_haptic *aw_haptic, ssize_t len, char *buf)
{
	uint8_t i = 0;
	uint8_t reg_array[AW869X_REG_NUM_F0_3 + 1] = {0};

	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_ID, reg_array, AW869X_REG_RTP_DATA);
	...
	haptic_hv_i2c_reads(aw_haptic, (AW869X_REG_RAMDATA + 1), &reg_array[AW869X_REG_RAMDATA + 1],
			    (AW869X_REG_NUM_F0_3 - AW869X_REG_RAMDATA));
...
static void aw869x_get_lra_resistance(struct aw_haptic *aw_haptic)
{
	...
	aw869x_stop(aw_haptic);
	aw869x_ram_init(aw_haptic, true);
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_SYSCTRL, AW869X_BIT_SYSCTRL_BST_MODE_MASK,
				 AW869X_BIT_SYSCTRL_BST_MODE_BYPASS);
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_ANACTRL, AW869X_BIT_ANACTRL_HD_PD_MASK,
				 AW869X_BIT_ANACTRL_HD_HZ_EN);
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_D2SCFG, AW869X_BIT_D2SCFG_CLK_ADC_MASK,
				 AW869X_BIT_D2SCFG_CLK_ASC_1P5MHZ);
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_DETCTRL,
				 (AW869X_BIT_DETCTRL_RL_OS_MASK & AW869X_BIT_DETCTRL_DIAG_GO_MASK),
				 (AW869X_BIT_DETCTRL_RL_DETECT |
				  AW869X_BIT_DETCTRL_DIAG_GO_ENABLE));
	usleep_range(3000, 3500);
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_RLDET, &reg_val, AW_I2C_BYTE_ONE);
...
static int aw869x_read_f0(struct aw_haptic *aw_haptic)
{
	...
	/* lra_f0 */
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_F_LRA_F0_H, reg_val, AW_I2C_BYTE_TWO);
	...
	aw_haptic->f0 = (uint32_t)AW869X_F0_FORMULA(f0_reg, aw_haptic->info.f0_coeff);
	aw_info("lra_f0=%d", aw_haptic->f0);
#else
	/* cont_f0 */
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_F_LRA_CONT_H, reg_val, AW_I2C_BYTE_TWO);
	f0_reg = (reg_val[0] << 8) | (reg_val[1] << 0);
	if (!(f0_reg * aw_haptic->info.f0_coeff)) {
		aw_haptic->f0 = 0;
...
static void aw869x_read_beme(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_WAIT_VOL_MP, &reg_val, AW_I2C_BYTE_ONE);
	aw_haptic->max_pos_beme = (reg_val << 0);
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_WAIT_VOL_MN, &reg_val, AW_I2C_BYTE_ONE);
...
static uint8_t aw869x_rtp_get_fifo_afs(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_SYSST, &reg_val, AW_I2C_BYTE_ONE);
...
static void aw869x_upload_lra(struct aw_haptic *aw_haptic, uint32_t flag)
{
	...
	case AW_WRITE_ZERO:
		...
		haptic_hv_i2c_writes(aw_haptic, AW869X_REG_TRIM_LRA, &zero, AW_I2C_BYTE_ONE);
		break;
	case AW_F0_CALI_LRA:
		aw_info("f0_cali_lra=%d", aw_haptic->f0_cali_data);
		f0 = (uint8_t)aw_haptic->f0_cali_data;
		haptic_hv_i2c_writes(aw_haptic, AW869X_REG_TRIM_LRA, &f0, AW_I2C_BYTE_ONE);
		break;
	case AW_OSC_CALI_LRA:
		aw_info("rtp_cali_lra=%d", aw_haptic->osc_cali_data);
		osc = (uint8_t)aw_haptic->osc_cali_data;
		haptic_hv_i2c_writes(aw_haptic, AW869X_REG_TRIM_LRA, &osc, AW_I2C_BYTE_ONE);
		break;
	default:
		break;
	}
...
static uint64_t  aw869x_get_theory_time(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_PWMDBG, &reg_val, AW_I2C_BYTE_ONE);
	fre_val = reg_val & (~AW869X_BIT_PWMDBG_PWM_MODE_MASK);
	if (fre_val == AW869X_BIT_PWMDBG_PWM_12K)
		theory_time = aw_haptic->rtp_len * 1000 / 12; /*12K */
	if (fre_val == AW869X_BIT_PWMDBG_PWM_24K)
		theory_time = aw_haptic->rtp_len * 1000 / 24; /*24K */
	if (fre_val == AW869X_BIT_PWMDBG_PWM_48K)
...
static uint8_t aw869x_osc_read_status(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_DBGSTAT, &reg_val, AW_I2C_BYTE_ONE);
...
static void aw869x_cont_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_DATCTRL,
				 (AW869X_BIT_DATCTRL_FC_MASK & AW869X_BIT_DATCTRL_LPF_ENABLE_MASK),
				 (AW869X_BIT_DATCTRL_FC_1000HZ | AW869X_BIT_DATCTRL_LPF_ENABLE));
	/* cont config */
	...
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_TD_H, cont_td, AW_I2C_BYTE_TWO);
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_TSET, &aw_haptic->info.tset, AW_I2C_BYTE_ONE);
	/* zero cross */
	cont_zc[0] = (uint8_t)(aw_haptic->info.cont_zc_thr >> 8);/* cont_zc_h */
	cont_zc[1] = (uint8_t)(aw_haptic->info.cont_zc_thr >> 0);/* cont_zc_l */
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_ZC_THRSH_H, cont_zc, AW_I2C_BYTE_TWO);

	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_BEMF_NUM, AW869X_BIT_BEMF_NUM_BRK_MASK,
				 aw_haptic->info.cont_num_brk);
	/* 35*171us=5.985ms */
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_TIME_NZC, &time_nzc, AW_I2C_BYTE_ONE);
	/* f0 driver level */
	drv_lvl[0] = aw_haptic->info.cont_drv_lvl;
	drv_lvl[1] = aw_haptic->info.cont_drv_lvl_ov;
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_DRV_LVL, drv_lvl, AW_I2C_BYTE_TWO);
	/* cont play go */
...
static int aw869x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_NUM_F0_1, f0_param, AW_I2C_BYTE_THREE);
...
static int aw869x_set_pref0(struct aw_haptic *aw_haptic, uint32_t f0_pre)
{
	...
	if (!(f0_pre && aw_haptic->info.f0_coeff)) {
		aw_err("Divisor is 0! f0_pre=%d, f0_coeff=%d", f0_pre, aw_haptic->info.f0_coeff);
		return -EPERM;
	}
	f0_reg = 1000000000 / (f0_pre * aw_haptic->info.f0_coeff);
	reg_val[0] = (uint8_t)((f0_reg >> 8) & 0xff);
	reg_val[1] = (uint8_t)((f0_reg >> 0) & 0xff);
	haptic_hv_i2c_writes(aw_haptic, AW869X_REG_F_PRE_H, reg_val, AW_I2C_BYTE_TWO);
...
static void aw869x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	/* Get gain */
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_DATDBG, reg_val, AW_I2C_BYTE_ONE);
	aw_haptic->gain = reg_val[0];
	/* Get wave_seq */
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_WAVSEQ1, reg_val, AW_I2C_BYTE_EIGHT);
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_ANADBG, AW869X_BIT_ANADBG_IOC_MASK,
				 AW869X_BIT_ANADBG_IOC_4P65A);
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_ANACTRL, AW869X_BIT_ANACTRL_LRA_SRC_MASK,
...
static void aw869x_interrupt_setup(struct aw_haptic *aw_haptic)
{
	...
	/* edge int mode */
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_DBGCTRL, AW869X_BIT_DBGCTRL_INT_MODE_MASK,
				 AW869X_BIT_DBGCTRL_INT_MODE_EDGE);
	/* int enable */
	haptic_hv_i2c_write_bits(aw_haptic, AW869X_REG_SYSINTM,
				 (AW869X_BIT_SYSINTM_BSTERR_MASK & AW869X_BIT_SYSINTM_OV_MASK &
				  AW869X_BIT_SYSINTM_UVLO_MASK & AW869X_BIT_SYSINTM_OCD_MASK &
...
static int aw869x_get_irq_state(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_SYSST, &int_sts, AW_I2C_BYTE_ONE);
	aw_dbg("reg SYSST=0x%x", int_sts);
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_SYSINT, &reg_val, AW_I2C_BYTE_ONE);
	aw_dbg("reg SYSINT=0x%x", reg_val);
	haptic_hv_i2c_reads(aw_haptic, AW869X_REG_DBGSTAT, &dbg_val, AW_I2C_BYTE_ONE);
 ...
 *
 *****************************************************/
static void aw869x_parse_dt(struct device *dev, struct aw_haptic *aw_haptic, struct device_node *np)
{
	uint8_t bstdbg[6];
	...
	val = of_property_read_u8(np, "aw869x_cont_drv_lev", &aw_haptic->info.cont_drv_lvl);
	if (val != 0)
		aw_info("aw869x_cont_drv_lev not found");
	val = of_property_read_u8(np, "aw869x_cont_drv_lvl_ov", &aw_haptic->info.cont_drv_lvl_ov);
	if (val != 0)
		aw_info("aw869x_cont_drv_lvl_ov not found");
	val = of_property_read_u32(np, "aw869x_cont_td", &aw_haptic->info.cont_td);
	if (val != 0)
		aw_info("aw869x_cont_td not found");
	val = of_property_read_u32(np, "aw869x_cont_zc_thr", &aw_haptic->info.cont_zc_thr);
	if (val != 0)
		aw_info("aw869x_cont_zc_thr not found");
	val = of_property_read_u8(np, "aw869x_cont_num_brk", &aw_haptic->info.cont_num_brk);
	if (val != 0)
		aw_info("aw869x_cont_num_brk not found");
	val = of_property_read_u32(np, "aw869x_f0_coeff", &aw_haptic->info.f0_coeff);
	if (val != 0)
	...
	val = of_property_read_u8(np, "aw869x_r_spare", &aw_haptic->info.r_spare);
	if (val != 0)
		aw_info("aw869x_r_spare not found");
	val = of_property_read_u8_array(np, "aw869x_bstdbg", bstdbg, ARRAY_SIZE(bstdbg));
	if (val != 0)
	...
	val = of_property_read_u32(np, "aw869x_bst_vol_default", &aw_haptic->info.bst_vol_default);
	if (val != 0)
		aw_info("aw869x_bst_vol_default not found");
}

static ssize_t cont_td_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	len += snprintf(buf + len, PAGE_SIZE - len, "aw869x cont delay time = 0x%04x\n",
...
static ssize_t cont_drv_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "aw869x cont drv level = %d\n",
			aw_haptic->info.cont_drv_lvl);
	len += snprintf(buf + len, PAGE_SIZE - len, "aw869x cont drv level overdrive= %d\n",
...
static ssize_t cont_num_brk_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "aw869x cont break num = %d\n",
			aw_haptic->info.cont_num_brk);
...
static ssize_t cont_zc_thr_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "aw869x cont zero cross thr = 0x%04x\n",
			  ...
			  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u %u %u %u %u %u", &databuf[0], &databuf[1], &databuf[2], &databuf[3],
	    &databuf[4], &databuf[5]) == 6) {
		aw_info("%d, %d, %d, %d, %d, %d", databuf[0], databuf[1], databuf[2], databuf[3],
			databuf[4], databuf[5]);
		if (databuf[0] > 3)
			databuf[0] = 3;
		if (databuf[0] > 0)
		...
		if (databuf[4] > aw_haptic->ram.ram_num || databuf[5] > aw_haptic->ram.ram_num) {
...
static int aw869x_creat_node(struct aw_haptic *aw_haptic)
{
	...
	ret = sysfs_create_group(&aw_haptic->vib_dev.dev->kobj, &aw869x_vibrator_attribute_group);
	if (ret < 0)
