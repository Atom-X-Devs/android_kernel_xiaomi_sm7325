...
#ifdef CONFIG_MACH_XIAOMI
		mi_disp_feature_event_notify_by_type(mi_get_disp_id(display),
...
#ifdef CONFIG_MACH_XIAOMI
	mi_disp_feature_event_notify_by_type(mi_get_disp_id(display), MI_DISP_EVENT_FPS,
			sizeof(timing.refresh_rate), timing.refresh_rate);

	if (display->panel->cur_mode->timing.refresh_rate != timing.refresh_rate)
		mi_disp_feature_sysfs_notify(mi_get_disp_id(display), MI_SYSFS_DYNAMIC_FPS);
#endif
