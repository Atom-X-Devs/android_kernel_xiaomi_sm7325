...
int tfa987x_irq_handle(struct tfa_device *tfa)
{
	...
	for (offset = 0; offset < tfa9873_irq_max; offset++) {
		if ((tfa->interrupt_enable[0] & (1 << offset)) &&
		    (value & (1 << offset)))
			pr_err("[IRQ 0x%x] %s\n", tfa->slave_address,
			       irq_msg[offset]);
	}
...
enum Tfa98xx_Error tfa_dsp_msg_read_rpc(struct tfa_device *tfa, int length,
					unsigned char *bytes)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
enum Tfa98xx_Error tfa_dsp_msg(struct tfa_device *tfa, int length24,
			       const char *buf24)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	int lastmessage = 0;
...
enum Tfa98xx_Error tfa_dsp_msg_read(struct tfa_device *tfa, int length24,
...
enum Tfa98xx_Error tfa_mem_read(struct tfa_device *tfa,
				unsigned int start_offset, int num_words,
				int *pValues)
{
	enum Tfa98xx_Error error;

	error = (tfa->dev_ops.tfa_mem_read)(tfa, start_offset, num_words,
					    pValues);
	if (error != Tfa98xx_Error_Ok)
		error = (enum Tfa98xx_Error)(
			error +
...
enum Tfa98xx_Error tfa_mem_write(struct tfa_device *tfa, unsigned short address,
...
enum Tfa98xx_Error tfa_dsp_msg_rpc(struct tfa_device *tfa, int length,
				   const char *buf)
{
	enum Tfa98xx_Error error;
					...
					unsigned char data[])
{
	...
	error = tfa_dsp_cmd_id_write_read(
		tfa, MODULE_FRAMEWORK, FW_PAR_ID_GET_API_VERSION, 3, api_ver);
	buffer[nr++] = tfa->spkr_select;
	buffer[nr++] = module_id + 128;
	buffer[nr++] = param_id;

	if ((api_ver[1] < TFA_API_SBFW_8_09_00) ||
	    (api_ver[0] == 2)) { //(api_ver[0] == 2) is for 88 device
				      ...
				      unsigned char *pFirmwareVersion)
{
	...
	if (!tfa->is_probus_device) {
		int buffer;
		err = tfa_mem_read(tfa, FW_VAR_API_VERSION, 1,
				   (int *)(&buffer));
		if (err) {
			pr_debug(
...
#ifdef __KERNEL__
		if ((tfa->dynamicTDMmode == 3) &&
		    tfa_dev_set_tdm_bitwidth(tfa, tfa->bitwidth))
			return Tfa98xx_Error_Fail;
#endif //
	} else {
		pr_debug(
					 ...
					 int *calibrateDone)
{
	...
	*calibrateDone = 0;
	...
	while ((*calibrateDone != 1) &&
	       (tries < TFA98XX_API_WAITRESULT_NTRIES)) {
		err = tfa_mem_read(tfa, TFA_FW_XMEM_CALIBRATION_DONE, 1,
				   calibrateDone);
		if (*calibrateDone == -1)
			break;
		tries++;
			     ...
			     int vstep)
{
	...
	if (tfa->bus != 0) { /* non i2c  */
		...
		if (strstr(tfaContProfileName(tfa->cnt, tfa->dev_idx,
			...
			tfa_dev_set_state(
				tfa, TFA_STATE_OPERATING | TFA_STATE_MUTE, 0);
#else
...
enum tfa_error tfa_dev_stop(struct tfa_device *tfa)
{
	...
	while ((manstate >= 4) && (retry < 100)) {
		pr_debug(
			"%s  waitting state machine goto powerdown.  MANSTATE=%d\n",
			__func__, manstate);
	...
	if (tfa_dev_mtp_get(tfa, TFA_MTP_OTC) &&
		...
		if (!tfa->is_probus_device) {
			...
			while ((calibrateDone != 1) &&
			       (tries < TFA98XX_API_WAITRESULT_NTRIES)) {
				error = tfa_mem_read(
					tfa, TFA_FW_XMEM_CALIBRATION_DONE, 1,
					&calibrateDone);
#ifndef CONFIG_MACH_XIAOMI
				if (calibrateDone == 1)
#else
...
enum Tfa98xx_Error tfa_dsp_partial_coefficients(struct tfa_device *tfa,
	...
	case TFA_STATE_INIT_CF: /* coolflux HW access possible (~initcf) */
		...
		do {
			err = (enum tfa_error)tfa98xx_dsp_system_stable(
				tfa,
				&ready); //add (enum tfa_error) for compile normal.
			if (err != tfa_error_ok)
				return err;
			if (ready)
				break;
...
#ifdef __KERNEL__
void tfa_dev_get_tdm_add(struct tfa_device *tfa, uint16_t *tdme,
			 uint16_t *tnbck, uint16_t *tslln, uint16_t *tsize)
{
...
int tfa_dev_set_tdm_bitwidth(struct tfa_device *tfa, int width) /*SACAS-26*/
{
	...
	case 24: /* 24-bit sample in 32-bit slot */
		nbck = 2;
		slotlen = 31;
		samplesize = (tfa->tfa_family == 1)	  ? 23 :
			     (tfa->is_probus_device == 1) ? 31 :
							    23;
		break;
	...
	if (tfa->tfa_family == 2) {
		...
		tfa_dev_get_tdm_add(tfa, &tdme, &tdmnbck, &tdmslln,
				    &tdmssize); /*SACAS-26*/
		/* stop tdm */
...
void tfa_adapt_noisemode(struct tfa_device *tfa)
{
	int i, avbatt;
	long total_bats = 0;
	if ((tfa_get_bf(tfa, 0x5900) == 0) ||
	    (tfa_get_bf(tfa, TFA9873_BF_LP1) == 1)) {
		if (tfa->verbose)
			pr_debug(
		    ...
		    TFA9873_BF_LNMODE)) //640 corresponds to 3.4 volt, MCH_TO_TEST
	{
		...
		pr_debug("\navbatt= %d--Applying high noise gain\n", avbatt);
		   ...
		   tfa_get_bf(tfa, TFA9873_BF_LNMODE)) {
...
int tfa_plop_noise_interrupt(struct tfa_device *tfa, int profile, int vstep)
{
	...
	if (tfa_irq_get(tfa, tfa9912_irq_stnoclk)) {
		...
		if (no_clk == 1) {
			...
			err = (enum Tfa98xx_Error)tfa_dev_start(
				tfa, profile,
				vstep); //add (enum Tfa98xx_Error) for compile normal.
			if (err != Tfa98xx_Error_Ok) {
