 ...
 */
TfaProfileList_t *tfaContGetDevProfList(TfaContainer_t *cont, int devIdx,
					int profIdx)
{
	TfaDeviceList_t *dev;
	int idx, hit;
...
int tfacont_get_max_vstep(struct tfa_device *tfa, int prof_idx)
{
	...
	int vstep_count = 0;
	vp = (TfaVolumeStep2File_t *)tfacont_getfiledata(tfa, prof_idx,
							 volstepHdr);
	if (vp == NULL)
		return 0;
	...
	if (tfa->tfa_family == 2) {
		...
		vp3 = (struct TfaVolumeStepMax2File *)tfacont_getfiledata(
				  ...
				  enum TfaHeaderType type)
{
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscFile) {
			file = (TfaFileDsc_t *)(dev->list[i].offset +
						(uint8_t *)tfa->cnt);
			if (file != NULL) {
	...
	for (i = 0; i < prof->length; i++) {
		if (prof->list[i].type == dscFile) {
			file = (TfaFileDsc_t *)(prof->list[i].offset +
						(uint8_t *)tfa->cnt);
			if (file != NULL) {
				hdr = (TfaHeader_t *)file->data;
				if (hdr != NULL) {
...
static enum Tfa98xx_Error tfaContWriteVstep(struct tfa_device *tfa,
					    TfaVolumeStep2File_t *vp, int vstep)
{
	...
	if (vstep < vp->vsteps) {
		...
		err = tfa98xx_dsp_write_preset(tfa, sizeof(vp->vstep[0].preset),
				    ...
				    int vstep_idx, int vstep_msg_idx)
{
	...
	case msgHdr: /* generic DSP message */
		...
		err = tfa_dsp_msg(tfa, size,
				  (const char *)((TfaMsgFile_t *)hdr)->data);
		break;
	case volstepHdr:
		if (tfa->tfa_family == 2) {
			err = tfaContWriteVstepMax2(
				tfa, (TfaVolumeStepMax2File_t *)hdr, vstep_idx,
				vstep_msg_idx);
		} else {
			err = tfaContWriteVstep(
				tfa, (TfaVolumeStep2File_t *)hdr, vstep_idx);
		}
		break;
	case speakerHdr:
		if (tfa->tfa_family == 2) {
	...
	case presetHdr:
		...
		err = tfa98xx_dsp_write_preset(
			tfa, size,
			(const unsigned char *)((TfaPreset_t *)hdr)->data);
		break;
	case equalizerHdr:
		err = tfa_cont_write_filterbank(
			tfa, ((TfaEqualizerFile_t *)hdr)->filter);
		break;
	case patchHdr:
		size = hdr->size - sizeof(TfaPatch_t); // size is total length
		err = tfa_dsp_patch(
			tfa, size,
			(const unsigned char *)((TfaPatch_t *)hdr)->data);
		break;
	case configHdr:
		size = hdr->size - sizeof(TfaConfig_t);
		err = tfa98xx_dsp_write_config(
			tfa, size,
			(const unsigned char *)((TfaConfig_t *)hdr)->data);
		break;
	case drcHdr:
		if (hdr->version[0] == TFA_DR3_VERSION) {
			...
			err = tfa98xx_dsp_write_drc(
				tfa, size,
				((const unsigned char *)((TfaDrc_t *)hdr)->data +
				 381));
		}
		break;
	case infoHdr:
 ...
 */
static TfaDescPtr_t *tfa_cnt_get_dsc(TfaContainer_t *cnt,
				     TfaDescriptorType_t type, int dev_idx)
{
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == (uint32_t)type) {
			_this = (TfaDescPtr_t *)(dev->list[i].offset +
						 (uint8_t *)cnt);
			return _this;
		}
	}
...
enum Tfa98xx_Error tfaContWriteRegsDev(struct tfa_device *tfa)
{
	...
	for (i = 0; i < dev->length; i++) {
		...
		if (dev->list[i].type == dscBitfield) {
			bitF = (TfaBitfield_t *)(dev->list[i].offset +
						 (uint8_t *)tfa->cnt);
			err = tfaRunWriteBitfield(tfa, *bitF);
		}
		if (dev->list[i].type == dscRegister) {
			err = tfaRunWriteRegister(
				tfa, (TfaRegpatch_t *)(dev->list[i].offset +
						       (char *)tfa->cnt));
		}
...
enum Tfa98xx_Error tfaContWriteRegsProf(struct tfa_device *tfa, int prof_idx)
{
	...
	for (i = 0; i < prof->length; i++) {
		...
		if (prof->list[i].type == dscBitfield) {
			bitf = (TfaBitfield_t *)(prof->list[i].offset +
						 (uint8_t *)tfa->cnt);
			err = tfaRunWriteBitfield(tfa, *bitf);
		}
		if (prof->list[i].type == dscRegister) {
			err = tfaRunWriteRegister(
				tfa, (TfaRegpatch_t *)(prof->list[i].offset +
						       (char *)tfa->cnt));
		}
		if (err)
			break;
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscPatch) {
			...
			size = patchfile->hdr.size -
			       sizeof(TfaPatch_t); // size is total length
			err = tfa_dsp_patch(
				tfa, size,
 ...
 */
static void create_dsp_buffer_msg(struct tfa_device *tfa, TfaMsg_t *msg,
	...
	for (i = 0; i < dev->length; i++) {
		...
		if (dev->list[i].type == dscCfMem) {
			err = tfaRunWriteDspMem(
				tfa, (TfaDspMem_t *)(dev->list[i].offset +
						     (uint8_t *)tfa->cnt));
		}
					 ...
					 int vstep_idx)
{
	...
	for (i = 0; i < prof->length; i++) {
		...
		case dscCfMem:
			err = tfaRunWriteDspMem(
				tfa, (TfaDspMem_t *)(prof->list[i].offset +
						     (uint8_t *)tfa->cnt));
			break;
					   ...
					   TfaDescPtr_t *dsc)
{
	...
	case dscCfMem:
		err = tfaRunWriteDspMem(
			tfa,
			(TfaDspMem_t *)(dsc->offset + (uint8_t *)tfa->cnt));
		break;
	case dscBitfield:
	...
	case dscFilter:
		return tfaRunWriteFilter(
			tfa,
			(TfaContBiquad_t *)(dsc->offset + (uint8_t *)tfa->cnt));
		break;
	}
...
static enum Tfa98xx_Error get_sample_rate_info(struct tfa_device *tfa,
					       TfaProfileList_t *prof,
					       TfaProfileList_t *previous_prof,
	...
	for (i = 0; i < previous_prof->length; i++) {
		...
		if (i < previous_prof->length) {
			if (previous_prof->list[i].type == dscBitfield) {
				bitf = (TfaBitfield_t *)(previous_prof->list[i]
								 .offset +
							 (uint8_t *)tfa->cnt);
				if (bitf->field == TFA_FAM(tfa, AUDFS)) {
				       ...
				       int vstep_idx)
{
	...
	TfaProfileList_t *prof =
		tfaContGetDevProfList(tfa->cnt, tfa->dev_idx, prof_idx);
	TfaProfileList_t *previous_prof = tfaContGetDevProfList(
...
TfaProfileList_t *tfaContNextProfile(TfaProfileList_t *prof)
{
	...
	next = this + listlength +
	       sizeof(TfaProfileList_t); // - sizeof(TfaDescPtr_t);
...
{
	TfaLiveDataList_t *nextlivedata =
		(TfaLiveDataList_t *)((char *)livedata +
				      (livedata->length * 4) +
	...
	*hw_feature_register = -1;
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscFeatures) {
			features = (TfaFeatures_t *)(dev->list[i].offset +
						     (uint8_t *)tfa->cnt);
