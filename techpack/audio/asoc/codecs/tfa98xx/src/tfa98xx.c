...
#include "config.h"
#include "tfa98xx.h"
#include "tfa98xx_tfafieldnames.h" /* required for enum tfa9912_irq */
...
/* Be carefull:
 * setting pcm_sample_format to 3 means TDM settings will be dynamically adapted,
 * please do not set the HW TDM Setting in the container file in case of dynamic
 * sample format seletcion
 */
				      ...
				      loff_t *ppos)
{
	...
	if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
		...
		error = tfa_dsp_msg(tfa98xx->tfa, msg_file->size,
				    msg_file->data);
		if (error != Tfa98xx_Error_Ok) {
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	ret = snd_soc_add_component_controls(tfa98xx->codec, tfa98xx_controls,
					     mix_index);
...
static void tfa98xx_add_widgets(struct tfa98xx *tfa98xx)
{
	struct snd_soc_dapm_context *dapm =
		snd_soc_component_get_dapm(tfa98xx->codec);
		...
		if (do_sync) {
			...
			list_for_each_entry (tfa98xx, &tfa98xx_device_list,
				...
				if (tfa98xx->tfa->tfa_family == 1 ||
				    tfa98xx->tfa->dev_ops.tfa_status != NULL)
					queue_delayed_work(
						tfa98xx->tfa98xx_wq,
