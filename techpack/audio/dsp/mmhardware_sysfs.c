	...
	struct kobj_attribute *a, char *buf)
{
	...
	if ((mi->mm_id == MM_HW_ADSP) || (mi->mm_id == MM_HW_CODEC)) {
		if (!mi->on_register)
			pr_info("%s: 0x%x is not registered\n", __func__, mi->mm_id);
		return sprintf(buf, "%d\n", mi->on_register);
	} else if ((mi->mm_id == MM_HW_PA_1) || (mi->mm_id == MM_HW_PA_2) ||
			(mi->mm_id == MM_HW_PA_3) || (mi->mm_id == MM_HW_PA_4) ||
			(mi->mm_id == MM_HW_PA_5) || (mi->mm_id == MM_HW_PA_6) ||
			(mi->mm_id == MM_HW_PA_7) || (mi->mm_id == MM_HW_PA_8)) {
		if (!mi->on_register)
			pr_info("%s: 0x%x is not registered\n", __func__, mi->mm_id);
...
int on_calibration_under_mmsysfs(enum hardware_id mm_id, const char *name)
{
	...
	while (a) {
		...
		if (mm_id == mi->mm_id) {
			find_id = 1;
			if ((mi->mm_id == MM_HW_PA_1) || (mi->mm_id == MM_HW_PA_2) ||
				(mi->mm_id == MM_HW_PA_3) || (mi->mm_id == MM_HW_PA_4) ||
				(mi->mm_id == MM_HW_PA_5) || (mi->mm_id == MM_HW_PA_6) ||
				(mi->mm_id == MM_HW_PA_7) || (mi->mm_id == MM_HW_PA_8))
				mi->on_calibration = 1;
