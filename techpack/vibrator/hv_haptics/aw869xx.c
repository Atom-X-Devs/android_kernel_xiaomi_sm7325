...
static int aw869xx_offset_cali(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW869XX_REG_SYSCTRL7, &reg_val[2], AW_I2C_BYTE_ONE);
	...
	aw869xx_ram_init(aw_haptic, false);
	reg_val[2] &= (~AW869XX_BIT_SYSCTRL7_D2S_GAIN_MASK);
	d2s_gain = aw869xx_select_d2s_gain(reg_val[2]);
	if (d2s_gain <= 0) {
