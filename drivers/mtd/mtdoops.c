...
#ifdef CONFIG_MACH_XIAOMI
static void mtdoops_add_reason(char *oops_buf, enum mtd_dump_reason reason,
			       enum mtdoops_log_type type, int index,
			       int nextpage)
{
	char str_buf[200] = { 0 };
	int ret_len = 0;
	...
	if (nextpage > 1)
		ret_len = snprintf(
			...
			log_type[type], ts.tm_year + 1900, ts.tm_mon + 1,
...
static void mtdoops_add_pmsg_head(char *oops_buf, enum mtdoops_log_type type)
{
	...
	ret_len = snprintf(
		str_buf, 80,
		"\n```\n#### LOG TYPE:%s\n#####%04ld-%02d-%02d %02d:%02d:%02d\t\n```c\t\n",
		log_type[type], ts.tm_year + 1900, ts.tm_mon, ts.tm_mday,
			    ...
			    enum mtd_dump_reason reason)
{
	...
	struct mtdoops_context *cxt =
		container_of(dumper, struct mtdoops_context, dump);
	size_t ret_len = 0;
	...
	mtdoops_add_reason(cxt->oops_buf + MTDOOPS_HEADER_SIZE, reason,
			   MTDOOPS_TYPE_DMESG, cxt->nextcount, cxt->nextpage);
	...
	pr_info("mtdoops: mtdoops_do_dump pmsg paddr = 0x%lx\n", pmsg_buffer_start);
	...
	if (p_hdr->sig == 0x43474244) {
		...
		mtdoops_add_pmsg_head(cxt->oops_buf +
					      (MTDOOPS_HEADER_SIZE + ret_len),
				      MTDOOPS_TYPE_PMSG);
	} else
	...
	pr_info("mtdoops: mtdoops_do_dump() finish\n");
}
...
#define parse_u32(name, field, default_value)                                  \
	{                                                                      \
		ret = mtdoops_parse_dt_u32(pdev, name, default_value, &value); \
		if (ret < 0)                                                   \
			return ret;                                            \
		field = value;                                                 \
	}
...
#undef parse_u32
	...
	pr_info("mtdoops: pares mtd_dt, mem_address = 0x%x, mem_size = 0x%x \n",
		cxt->pmsg_data.mem_address, cxt->pmsg_data.mem_size);
	pr_info("mtdoops: pares mtd_dt, pmsg_size = 0x%x \n",
		cxt->pmsg_data.pmsg_size);
