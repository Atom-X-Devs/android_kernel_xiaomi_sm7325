			   ...
			   struct goodix_ext_module *module)
{
	...
	if (QUERYBIT(gsx_gesture->gesture_type, gs_event.gesture_type)) {
		...
		if (cd->double_wakeup && gs_event.gesture_type == 0xcc) {
			ts_info("gesture report double tap");
			input_report_key(cd->input_dev, KEY_WAKEUP, 1);
			input_sync(cd->input_dev);
			input_report_key(cd->input_dev, KEY_WAKEUP, 0);
			input_sync(cd->input_dev);
		}
		if (cd->aod_status && gs_event.gesture_type == 0x4c) {
			ts_info("gesture report single tap");
				      ...
				      struct goodix_ext_module *module)
{
	...
	if (!cd->gesture_enabled || hw_ops == NULL) {
		ts_info("enter sleep mode");
		return EVT_CONTINUE;
	}
...
static int __init goodix_gsx_gesture_init(void)
{
	...
	if (!def_kobj || !def_kobj_type) {
		ts_err("can't get default kobj or default_kobj_type, exit!");
		return -EIO;
	}
