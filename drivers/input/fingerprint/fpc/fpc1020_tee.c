...
enum {
	FPC_GPIO_NO_DEFAULT 	= -1,
	FPC_GPIO_NO_DEFINED 	= -2,
	FPC_GPIO_REQUEST_FAIL	= -3,
};
...
static int request_vreg_gpio(struct fpc1020_data *fpc1020, bool enable)
{
	...
	if (enable && !fpc1020->gpios_requested) {
		...
		rc = devm_request_threaded_irq(dev, gpio_to_irq(fpc1020->irq_gpio), NULL,
							fpc1020_irq_handler, fpc1020->irqf, dev_name(dev), fpc1020);
		if (rc) {
 ...
 * Dummy attributes
 */
#define FPC_DUMMY_ATTR(name) 						\
static inline ssize_t name(struct device *dev,      \
		       struct device_attribute *attr,       \
		       const char *buf, size_t count)		\
{ 													\
	return count; 									\
...
static int device_prepare(struct fpc1020_data *fpc1020, bool enable)
{
	...
	if (enable && !fpc1020->prepared) {
		...
		if (regulator_is_enabled(fpc1020->vreg)) {
			dev_info(dev, "fp_vdd_vreg is already enabled!\n");
		} else {
			rc = regulator_enable(fpc1020->vreg);
			if (rc) {
				dev_err(dev, "error enabling fp_vdd_vreg!\n");
				regulator_put(fpc1020->vreg);
				fpc1020->vreg = NULL;
				goto exit;
			}
		}
		...
		rc = regulator_disable(fpc1020->vreg);
		if (rc) {
...
static DEVICE_ATTR(irq_enable, S_IWUSR | S_IRUSR
	| S_IRGRP | S_IWGRP, NULL, irq_enable_set);

FPC_DUMMY_ATTR(fingerdown_wait_set);
static DEVICE_ATTR(fingerdown_wait, S_IWUSR, NULL, fingerdown_wait_set);

FPC_DUMMY_ATTR(vendor_update);
static DEVICE_ATTR(vendor, S_IWUSR, NULL, vendor_update);

FPC_DUMMY_ATTR(clk_enable_set);
static DEVICE_ATTR(clk_enable, S_IWUSR, NULL, clk_enable_set);

FPC_DUMMY_ATTR(power_cfg_set);
static DEVICE_ATTR(power_cfg, S_IWUSR, NULL, power_cfg_set);

FPC_DUMMY_ATTR(regulator_enable_set);
...
static inline int reset_gpio_res(struct fpc1020_data *fpc1020)
{
	if (!fpc1020)
		return -ENOMEM;
