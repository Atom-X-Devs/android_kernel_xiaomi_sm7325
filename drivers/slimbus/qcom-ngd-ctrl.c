				  ...
				  struct slim_msg_txn *txn)
{
	...
	*pbuf = 0;
	...
	if (!timeout) {
		SLIM_ERR(ctrl, "TX timed out:MC:0x%x,mt:0x%x", txn->mc,
					txn->mt);
		mutex_unlock(&ctrl->tx_lock);
		return -ETIMEDOUT;
	}
	...
	if (usr_msg) {
		...
		if (!timeout) {
			SLIM_ERR(ctrl, "TX timed out:MC:0x%x,mt:0x%x",
				txn->mc, txn->mt);
			mutex_unlock(&ctrl->tx_lock);
			return -ETIMEDOUT;
		}
				       ...
				       struct slim_msg_txn *txn)
{
	...
	if (!timeout) {
		SLIM_ERR(dev, "TX timed out:MC:0x%x,mt:0x%x", txn->mc,
				txn->mt);
		return -ETIMEDOUT;
	}
	return 0;
...
static int qcom_slim_ngd_disable_stream(struct slim_stream_runtime *rt)
{
	...
	if (ret) {
		slim_free_txn_tid(ctrl, &txn);
		SLIM_ERR(dev, "TX timed out:MC:0x%x,mt:0x%x ret:%d\n",
				txn.mc,	txn.mt, ret);
		return ret;
	}
...
static int qcom_slim_ngd_enable(struct qcom_slim_ngd_ctrl *ctrl, bool enable)
{
	if (enable) {
		...
		if (ret) {
			SLIM_ERR(ctrl, "error adding slim controller\n");
			return ret;
		}

		SLIM_INFO(ctrl, "SLIM controller Registered\n");
	} else {
		qcom_slim_qmi_exit(ctrl);
		slim_unregister_controller(&ctrl->ctrl);
		SLIM_INFO(ctrl, "SLIM controller Un-Registered\n");
	}
