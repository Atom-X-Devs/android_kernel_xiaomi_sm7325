...
static void tx_macro_hs_unmute_dwork(struct work_struct *work)
{
	...
	struct tx_macro_priv *tx_priv = container_of(delayed_work, struct tx_macro_priv, tx_hs_unmute_dwork);;

	snd_soc_component_update_bits(tx_priv->component, BOLERO_CDC_TX2_TX_VOL_CTL,
...
void bolero_tx_macro_mute_hs(void)
{
	...
	if (delayed_work_pending(&g_tx_priv->tx_hs_unmute_dwork)) {
		dev_err(component->dev, "there is already a work, give up unmute\n");
		return;
	}
